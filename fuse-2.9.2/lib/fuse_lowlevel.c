...
static void do_syncdelete(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	char *name = (char *) inarg;
	int *control_flags = name + strlen(name) + 1;
	if (req->f->op.syncdelete)
	{
		req->f->op.syncdelete(req, nodeid, name, control_flags);
	}
	else
		fuse_reply_err(req, ENOSYS);
}
...
static void do_syncrmdir(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	char *name = (char *) inarg;
	int *control_flags = name + strlen(name) + 1;

	if (req->f->op.syncrmdir)
		req->f->op.syncrmdir(req, nodeid, name, control_flags);
	else
		fuse_reply_err(req, ENOSYS);
}
...
static void do_init(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	if (arg->minor >= 6) {
		...
		if (arg->flags & FUSE_DONT_MASK) {
			...
			f->conn.want |= FUSE_CAP_DONT_MASK;
		}
	...
	if (req->f->conn.proto_minor >= 18) {
		...
		f->conn.want |= FUSE_CAP_IOCTL_DIR;
	}
...
static struct {
	...
	[FUSE_READDIRPLUS]	= { (void *) 1,      "READDIRPLUS" },
	[FUSE_RENAME2]	= { (void *) 1,      "RENAME2" },
	[FUSE_SYNCDELETE]	= { do_syncdelete,      "SYNCDELETE" },
	[FUSE_SYNCDRMDIR]	= { do_syncrmdir,      "SYNCRMDIR" },
