...
char* path_translate(char *path, char* rel_path)
{
	char *nowptr = path;
	char *cache_path = calloc(1, strlen(path) + 8 + strlen(rel_path));
	char *temp;
	int offset = 0, i;
	if (!cache_path)
		return NULL;
...
free_exit:
	return strcat(cache_path, rel_path);
}

void yase_log_ul(unsigned long ul_num)
{
	struct File *pFile = NULL;

	pFile = fopen(FUSE_LOG, "a");

	if (pFile)
	{
		fprintf(pFile, "long: %lu\n", ul_num);
		fclose(pFile);
	}
...
void fuse_add_mountpoint(struct fuse *f, char *mpt)
{
	f->fs->mpt = mpt;
	return;
}

void fuse_add_fsname(struct fuse *f, char *fsname)
{
	f->fs->fsname = fsname;
	return;
}
...
{
	do {
		f->ctr = (f->ctr + 1) & 0xffffffffffffffff;
		if (!f->ctr)
			f->generation ++;
...
static int yas3_get_id(struct fuse *f, fuse_ino_t parent,
                  const char *name, const char *path, struct node *node)
{
	char *full_path;
    struct stat fstat;
    int ret, fino, gen = 0;
    ...
    //yase_log(path);
    full_path = path_translate(f->fs->mpt, path);

    if (!full_path)
    	goto error_out;

    //yase_log("path_translated");
    //yase_log(full_path);

    fino = open(full_path, O_RDONLY);
    if (fino < 0)
    	goto free_err_out;

    ret = stat(full_path, &fstat);
    if (ret != 0)
	{
		yase_log(full_path);
    	goto free_close_out;
    ...
    node->nodeid = fstat.st_ino;

    if ( ioctl(fino, FS_IOC_GETVERSION, &gen) == 0)
    {
	    //yase_log("generation");
	    //yase_log_int(gen);
	    node->generation = gen;
			      ...
			      const char *name, const char *path)
{
	...
	if (node == NULL) {
		...
		if(is_yas3(f) == 1)
		{
			if (yas3_get_id(f, parent, name, path, node) != 0)
			{
				free_node(f, node);
				node = NULL;
				goto out_err;
			...
			struct node *end)
{
	...
	if (wnode) {
		if (wnode->treelock != TREELOCK_WRITE)
		{
			yase_log("assert: wnode->treelock != TREELOCK_WRITE");
			yase_log_ul(nodeid);
		}
	...
	{
		if (node->treelock == 0 || 
			node->treelock == TREELOCK_WAIT_OFFSET || 
			node->treelock == TREELOCK_WRITE )
		{
			yase_log("assert: wnode->treelock");
			yase_log_int(node->treelock);
			yase_log_ul(nodeid);
		}
		assert(node->treelock != 0);
...
static void forget_node(struct fuse *f, fuse_ino_t nodeid, uint64_t nlookup, int from_kernel)
{
	struct node *node;
	...
	if(from_kernel)
	{
		...
		for (old_node = node->id_next; old_node != NULL; old_node = old_node->id_next)
			if (old_node->nodeid == nodeid)
				node = old_node;
	}
	...
	if(nlookup > node->nlookup)
	{
		yase_log_ul(nodeid);
		yase_log_int(nlookup);
			 ...
			 char *newname, size_t bufsize)
{
	...
	do {
		...
		do {
			f->hidectr ++;
			//snprintf(newname, bufsize, ".fuse_hidden%08x%08x",  (unsigned int) node->nodeid, f->hidectr);
			snprintf(newname, bufsize, ".fuse_hidden%08x%08x",  (unsigned int) (node->nodeid>>32),  
		       ...
		       struct fuse_entry_param *e, struct fuse_file_info *fi)
{
	...
	if (res == 0) {
		struct node *node;
		/*
		yase_log("find_node enter");
		if (f->fs->mpt)
			yase_log(f->fs->mpt);
		yase_log(path);
		*/
		node = find_node(f, nodeid, name, path);
		//yase_log("find_node finished");
		if (node == NULL)
			res = -ENOMEM;
		else {
			e->ino = node->nodeid;
...
static void fuse_lib_syncdelete(fuse_req_t req, fuse_ino_t parent,
                const char *name, int *control_flags)
{
    struct fuse *f = req_fuse_prepare(req);
    struct node *wnode;
    char *path;
...
static void fuse_lib_syncrmdir(fuse_req_t req, fuse_ino_t parent,
                const char *name, int *control_flags)
{
    struct fuse *f = req_fuse_prepare(req);
    struct node *wnode;
			    ...
			    struct fuse_file_info *fi)
{
	...
	//yase_log("fuse_lib_create enter");
	err = get_path_name(f, parent, name, &path);
	if (!err) {
		fuse_prepare_interrupt(f, req, &d);
		//yase_log("fuse_fs_create enter");
		err = fuse_fs_create(f->fs, path, mode, fi);
		//yase_log("fuse_fs_create finished");
		if (!err) {
			//yase_log("lookup_path enter");
			err = lookup_path(f, parent, name, path, &e, fi);
			//yase_log("lookup_path finished");
			if (err)
				fuse_fs_release(f->fs, path, fi);
...
static struct fuse_lowlevel_ops fuse_path_ops = {
	...
	.syncdelete = fuse_lib_syncdelete,
	.rmdir = fuse_lib_rmdir,
	.syncrmdir = fuse_lib_syncrmdir,
