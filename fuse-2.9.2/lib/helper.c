...
static struct fuse_chan *fuse_mount_common(const char *mountpoint,
					   struct fuse_args *args, char *fsname)
{
	struct fuse_chan *ch;
	int fd;
	...
	fd = fuse_mount_compat25(mountpoint, args, fsname);
	if (fd == -1)
		return NULL;
...
{
	return fuse_mount_common(mountpoint, args, NULL);
}
			       ...
			       int compat)
{
	...
	fsname = (char *) calloc(1, FS_NAME + 1);

	if (fsname == NULL)
		return NULL;

	memset(fsname, 0, FS_NAME + 1);

	res = fuse_parse_cmdline(&args, mountpoint, multithreaded, &foreground);
	if (res == -1)
		goto err_free_fsname;

	ch = fuse_mount_common(*mountpoint, &args, fsname);
	if (!ch) {
	...
	if(fsname)
		fuse_add_fsname(fuse, fsname);

	mpt = (char *) calloc(1, strlen(*mountpoint) + 1);

	if (mpt == NULL)
		goto err_unmount;

	memset(mpt, 0, strlen(*mountpoint) + 1);
	strncpy(mpt, *mountpoint, strlen(*mountpoint));
	fuse_add_mountpoint(fuse, mpt);
...
int fuse_mount_compat25(const char *mountpoint, struct fuse_args *args, char *fsname)
{
	return fuse_kern_mount(mountpoint, args, fsname);
}
