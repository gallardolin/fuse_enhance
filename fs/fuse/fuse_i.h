...
/* shared bucket mnt node */
typedef struct shared_bucket_mnt{
	char mnt_path[PATH_MAX];
	bool permissions;           // permission 0/1 : ro/rw
	struct list_head mnt_list;   // link with other rw node
	struct list_head *sub_list;  // link with all ro node under this rw node
} shared_bucket_mnt_t;

/* shared bucket sub node */
typedef struct shared_bucket_sub{
...
typedef struct yas3fs_dirty_range_t
{
	unsigned long long start[YAS3FS_N_DIRTY];
	unsigned long long end[YAS3FS_N_DIRTY];

	u32				   dirty_count;

} yas3fs_dirty_range_t;

struct dirty_record 
{
    struct rb_node   dirty_tree_node;
    struct list_head dirty_list_node;
    u64              ino;
    u32				 dirty_count;

    loff_t            start[YAS3FS_N_DIRTY];
    loff_t            end[YAS3FS_N_DIRTY];
...
struct dirty_range_root
{
    struct rb_root   *dirty_root;
    struct list_head *dirty_list_nood;
};
...
struct fuse_conn {
	...
	char *mount_path;
	char *mount_path_translated;
	struct list_head *shared_bucket_list;
	/** Ivan add for vfs enhance issue */
	int iscache;

	/** For async IOC to notify write range*/
	struct dirty_range_root dirty_root;
